...
const circleNavWrap = $("#circleNavWrap");
...
async function openAddChildModal(parentPath, parentDepth){
  ...
  const parentNode = nodeById(state.jtree, parentId);
  if(!parentNode) return;

  const siblings = (parentNode.children||[]).filter(cid=>!!nodeById(state.jtree, cid));
  ...
  comboInput.onkeydown = (e)=>{
    if(e.key==='Enter'){
      ...
      // User must explicitly click an item, or type an exact title/key.
      e.preventDefault();
      closeList();
  ...
  const idxInput = el('input', {className:'input', value:'', placeholder:'Індекс/позначення (необов’язково)'});
  ...
  while(true){
    ...
    let resolvedKey = selectedSheetKey;
    if(typed){
      const low = typed.toLowerCase();
      const m = (state.sheets||[]).find(s=>String(s.title||'').toLowerCase()===low || String(s.key||'').toLowerCase()===low);
  ...
  const childSheet = (state.sheets||[]).find(s=>s.key===selectedSheetKey);
...
async function setSpacePath(path){
  ...
  if(!state.spaceId){
    const roots = spaceChildren(state.spaces, null);
    if(roots[0]){ state.spacePath=[roots[0].id]; state.spaceId=roots[0].id; }
  }
  state.jtree = await ensureJournalTree(state.spaceId, state.sheets);
  state.journalPath = ensureValidJournalPath(state.jtree, []);
  // ensure top journal
  if(!state.journalPath.length){
    const topId = state.jtree?.topIds?.[0];
    if(topId) state.journalPath=[topId];
...
function fillSpaceSelect(spaces){
  ...
  var all = (spaces||[]).filter(function(s){ return s && s.kind==="space"; });
  var roots = spaceChildren(all, null);
  if(!roots.length) return;

  // Build numbering map (1, 1.1, 1.1.1 ...) based on current order in cfg.
  var numMap = {};
  function walk(parentId, prefix){
    var kids = spaceChildren(all, parentId);
  ...
  function labelSpace(s){
    var n = numMap[s.id] || "";
    return (n ? (n + " ") : "") + String(s.name||"");
  }

  // Ensure current path is valid.
  if(!Array.isArray(state.spacePath) || !state.spacePath.length){
    state.spacePath = [roots[0].id];
    state.spaceId = roots[0].id;
  }
  if(!spaceById(all, state.spacePath[0])){
    state.spacePath = [roots[0].id];
    state.spaceId = roots[0].id;
  }
  ...
  for(var i=0;i<roots.length;i++){
    var s = roots[i];
    spaceSelect.appendChild(el("option",{value:s.id, textContent:"📁 " + labelSpace(s)}));
  }
  spaceSelect.appendChild(el("option",{value:"__add__", textContent:"＋ Додати простір"}));
  spaceSelect.value = state.spacePath[0] || roots[0].id;
  spaceSelect.disabled = false;
  // Active highlight like journals (for orientation)
  spaceSelect.classList.toggle("nav-active", (state.spacePath||[]).length===1);

  // Subspace chain (dropdowns for deeper levels)
  if(subspaceChain) subspaceChain.innerHTML="";
  var parentId = spaceSelect.value;

  // Build dropdown for each already-selected depth > 0
  for(var depth=1; depth< (state.spacePath.length||0); depth++){
    var kids = spaceChildren(all, parentId);
    if(!kids.length) break;

    (function(depth, parentId, kids){
      ...
      for(var j=0;j<kids.length;j++){
        var k = kids[j];
        sel.appendChild(el("option",{value:k.id, textContent:"📁 " + labelSpace(k)}));
      }
      sel.appendChild(el("option",{value:"__add__", textContent:"＋ Додати підпростір"}));
      sel.value = state.spacePath[depth] || "";

      // Highlight: the deepest selected element
      sel.classList.toggle("nav-active", (state.spacePath.length-1)===depth);

      sel.onchange = async function(){
        if(sel.value === "__add__"){
          // revert selection before prompts
          sel.value = state.spacePath[depth] || "";
          var p = spaceById(all, parentId);
          var pname = p ? p.name : "Простір";
          var nm = (prompt("Назва підпростору у: " + pname, "Новий підпростір")||"").trim();
          if(!nm) return;
          var created = await addSubspace(parentId, nm);
          state.spaces = await ensureSpaces();
          var base = state.spacePath.slice(0, depth);
          base.push(parentId);
        ...
        var v = sel.value;
        var newPath = state.spacePath.slice(0, depth);
        if(v) newPath.push(v);
    ...
    parentId = state.spacePath[depth] || parentId;
  }

  // Always show ONE dropdown for children count (and allow creating child) if there are children under the current space.
  if(subspaceChain){
    var kids0 = spaceChildren(all, parentId);
    if(kids0.length){
      (function(parentId, kids0){
        var sel = el("select",{className:"select", style:"max-width:240px", ariaLabel:"Підпростір"});
        sel.appendChild(el("option",{value:"", textContent:String(kids0.length)}));
        for(var j=0;j<kids0.length;j++){
          var k = kids0[j];
          sel.appendChild(el("option",{value:k.id, textContent:"📁 " + labelSpace(k)}));
        }
        sel.appendChild(el("option",{value:"__add__", textContent:"＋ Додати підпростір"}));
        ...
        // Highlight: if there is no deeper selection, this is the active "next step"
        sel.classList.toggle("nav-active", (state.spacePath||[]).length>0 && (state.spacePath[state.spacePath.length-1]===parentId));
        ...
        sel.onchange = async function(){
          if(sel.value === "__add__"){
            ...
            var created = await addSubspace(parentId, nm);
            state.spaces = await ensureSpaces();
            var base = (state.spacePath||[]).slice();
            if(base.length===0) base = [parentId];
            var idx = base.lastIndexOf(parentId);
            base = (idx>=0) ? base.slice(0, idx+1) : [parentId];
            base.push(created.id);
            await setSpacePath(base);
            return;
          }
  ...
  // Subspace add button (small + right of last space dropdown)
  if(btnAddSubspace){
    btnAddSubspace.style.display = "inline-block";
  }
}


async function openAddRootJournalModal(){
  // Adds a new top-level journal node (sibling in the current space).
  const allSheets = (state.sheets||[]).slice();
  let selectedSheetKey = null;
  ...
  comboInput.addEventListener('focus', ()=>{ document.addEventListener('mousedown', outsideClose, true); });
  ...
  while(true){
    const res = await modalOpen({
      title: 'Додати журнал',
      bodyNodes:[
        el('div',{className:'muted', textContent:'Оберіть журнал зі списку (тип/шаблон).'}),
        el('div',{style:'height:10px'}),
        comboWrap,
        el('div',{style:'height:10px'}),
        el('div',{className:'muted', textContent:'Індекс (додаткове позначення для розрізнення)'}),
      ...
      actions:[ btn('Скасувати','cancel','btn'), btn('Створити','ok','btn btn-primary') ]
    });
    if(res?.type!=='ok') return;
  ...
  const sh = (state.sheets||[]).find(s=>s.key===selectedSheetKey);
  if(!sh){ alert('Не знайдено обраний журнал.'); return; }

  const topIds = Array.isArray(state.jtree?.topIds) ? state.jtree.topIds : [];
  const nextIdx = topIds.length + 1;
  const id = 'j_' + Date.now() + '_' + Math.random().toString(16).slice(2);
  state.jtree.nodes[id] = {
    id,
    parentId: null,
    sheetKey: sh.key,
    numPath: [nextIdx],
    title: (function(){
      var baseTitle = (sh && (sh.title||sh.name||sh.key)) || 'Журнал';
      var idx = String(idxInput.value||'').trim();
      return idx ? (baseTitle + ' ' + idx) : baseTitle;
    })(),
    children:[]
  };
  if(!Array.isArray(state.jtree.topIds)) state.jtree.topIds = [];
  state.jtree.topIds.push(id);
  await saveJournalTree(state.spaceId, state.jtree);

  state.journalPath = [id];
  await saveView();
  renderJournalNav();
  render();
}

// ensureValidJournalPath() is now in NavigationCore
...
function renderJournalNav(){
  ...
  for(const lvl of model.levels){
    ...
    const noneLbl = String((lvl.kids||[]).length);
    sel.appendChild(el("option",{value:"", textContent:noneLbl}));
    for(const k of lvl.kids){
    ...
    // Active highlight: when the current path ends at this depth+1.
    sel.classList.toggle("nav-active", state.journalPath.length === (lvl.depth+1));

    sel.onchange = async ()=>{
      if(sel.value === "__add__"){
        // revert selection and add a child under selected (if any) or under the parent at this level
        sel.value = lvl.selected || "";
        const parentPath = state.journalPath.slice(0, Math.max(lvl.depth, 1));
        // Add a new child under the parent of this dropdown (i.e. creates next sibling at this level).
        await openAddChildModal(parentPath, parentPath.length-1);
        return;
      }
      state.journalPath = setAtDepth(state.journalPath, lvl.depth, sel.value);
      state.journalPath = ensureValidJournalPath(state.jtree, state.journalPath);
      await saveView();
      renderJournalNav();
      render();
...
function renderCircleNav(model){
  if(!circleNavWrap) return;
  circleNavWrap.innerHTML="";
  ...
  // Space pick
  circleNavWrap.appendChild(mk({
    label: shortLabel(curSpace?.name||"S"),
    title:`📁 ${curSpace?.name||"Простір"}`,
    active:true,
    onClick: async()=>{
      const picked = await openPickModal({
        title:"Оберіть простір",
        items: flattenSpaces(),
        currentId: state.spaceId,
        addLabel: "＋ Додати простір"
      });
      ...
      if(picked==="__add__"){
        ...
        if(picked===state.spaceId) return;
        await setSpacePath(spacePathTo(picked));
        return;
      }
    }
  }));
  ...
  // Top journal pick
  const topId = state.journalPath[0] || model.topNodes[0]?.id;
  ...
  for(const lvl of model.levels){
    ...
    circleNavWrap.appendChild(mk({
      label: lbl,
      title: ttl,
      active: state.journalPath.length === (lvl.depth+1),
      onClick: async()=>{
        const picked = await openPickModal({
  ...
  circleNavWrap.appendChild(plus);
}
